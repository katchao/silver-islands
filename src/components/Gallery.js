...
function Gallery({ type }) {
    ...
    useEffect(() => {
        setImagesList(completeImagesList);
    }, [type]);
...
function ImageModalView({ shownImage, imagesList, handleThumbnailClick }) {
    ...
    const [thumbnailCarouselCanScroll, setThumbnailCarouselCanScroll] =
        useState(true);
        ...
        <div>
            ...
            <div className={styles.ModalThumbnailViewContainer}>
                ...
                >
                    {imagesList.map((image, i) => {
                        const selected =
                            image.displayName === shownImage.displayName;
                        return (
                            <Thumbnail
                                key={i}
                                src={image.thumbnailSrc}
